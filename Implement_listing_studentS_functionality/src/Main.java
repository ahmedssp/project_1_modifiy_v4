...
//        System.out.println(recordlist.get(0));
        ...
        for (int i = 0; i < recordlist.size(); i++) {
            ...
            for (int j = 0; j < line_array.length; j++) {
                if (line_array[j].length() > 10 && line_array[j].length() < 18) {
                    outputarray[i][j] = line_array[j] + "             ";
                } else if (j > 3) { if (line_array[j].length() > 3 && line_array[j].length() < 20) {
                    outputarray[i][j] = line_array[j] + "                      ";
                }
                    outputarray[i][j] = line_array[j] + "                ";
                } else {
    ...
    public static void main(String[] args) throws IOException {
        String rowdata_file_path="src/student_data_G.txt";
        String new_csv_file="file_test_csv.csv"; //type hear your new file and this is example name
